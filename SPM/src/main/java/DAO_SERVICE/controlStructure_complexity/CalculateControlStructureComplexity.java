			 ...
			 while(start != end+1) {
				 ...
				 if(stArray[start].contains("switch") &&  stArray[start].contains("(") && stArray[start].contains(")") && stArray[start].contains("{") ) {
					 ...
					 while(switchStart < switchEnd+1) {
						 System.out.println("Inside Switch : " + stArray[switchStart]);
						 if(stArray[switchStart].contains("case")) {
							 System.out.println("Case present");
							 numberOfCases++;
						 }
						 switchStart++;
					 }
		...
		for(x=0;x<functionList.size();x++) {
			...
			IndividualFunction f1 = functionList.get(x);
			 ...
			 while(start != end+1) {
				 ...
				 if((stArray[start].contains("if") && stArray[start].contains("(") && stArray[start].contains("{")) ||(stArray[start].contains("else") && stArray[start].contains("{")) ||  (stArray[start].contains("for")&& stArray[start].contains("(") && stArray[start].contains("(") && stArray[start].contains("{")) ||  (stArray[start].contains("while")&& stArray[start].contains("(") && stArray[start].contains("(")) ||  (stArray[start].contains("for")&& stArray[start].contains("(") && stArray[start].contains("(") && stArray[start].contains("{")) ||  (stArray[start].contains("do") && stArray[start].contains("{")) ||  (stArray[start].contains("switch")&& stArray[start].contains("(") && stArray[start].contains("(") && stArray[start].contains("{"))  ) {
...
//				 System.out.println("Linu number" + (start+1) + "Cnc is" +Cnc);
		 ...
		 ArrayList<StatementLine> displayCtcList = new ArrayList<>();
			...
			for(y=0;y<stArray.length;y++) {
			 ...
			 for(y=0;y<displayCtcList.size();y++) {
				 if(stArray[displayCtcList.get(y).getLineNumber() -1 ].isBlank()) {
					 displayCtcList.get(y).setComplexity(0);
				 }
			 }
		return displayCtcList;
	}
